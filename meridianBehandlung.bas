...
Sub B4xPage_Appear
	...
	Dim titelzeile As String = home.translate(Main.sprache,80)
	Dim meridiankurz As String
	If home.geklickterMeridian <> "" Then
		meridiankurz = home.geklickterMeridian
	Else
		meridiankurz = home.aktuellerMeridian
	End If
	Dim querytitel As String = "Select name from Meridiane where meridian='" & meridiankurz & "'"
	Dim titelzeileadd As String = Main.SQL.ExecQuerySingleResult(querytitel)
	titelzeile=titelzeile & " " & titelzeileadd
	home.setTitelzeile(titelzeile)
	
	query = "select * from viewbehandlung where meridian = '"& meridiankurz & "'"
'	Dim stromkurz As Boolean = home.settings.Get("stromkurz")
'	Dim queryadd As String
'	queryadd = " and ("
'	If stromkurz Then queryadd = queryadd&"kat = 'S' or"
'	Dim stromlang As Boolean = home.settings.Get("stromlang")
'	If stromlang Then queryadd = queryadd&" kat ='ST' or"
...
$"[Alignment=left][img dir="${xui.DefaultFolder}"img FileName=${bild} width=550/][/Alignment]"$
			End If
		End If
	If bild = "" And beschreibung <> "" And bild.Contains(".mp3")=False And kat<>("ST") Then
		code = code & _
$"[Alignment=center]${beschreibung}[/Alignment]"$
	End If
	#end if
	#if b4a
	If bild <> "" And bild.Contains(".mp3")=False And kat<>("ST") Then
		...
		code = code & _
$"[Alignment=center][img FileName=${bild} width=320/][/Alignment]
[Alignment=left]${beschreibung}[/Alignment]"$
			Else
...
Sub BBcodeview1_LinkClicked (URL As String)
	...
	'letzte 3 Buchstaben auswählen ob mp3, mp4
